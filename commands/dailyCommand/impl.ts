...
import { dateForHeader, dateFromDailyWhen, namefromDate } from "../../helpers/date-helper.ts";
import { exists } from "@std/fs/exists";
import { DailyWhen, dailyWhenFromString } from "../../options/whenOptions.ts";
...
export default async function (this: LocalContext, flags: DailyCommandFlags): Promise<void> {
    ...
    const when: DailyWhen = dailyWhenFromString(merged.when ?? flags.when);
    const date = dateFromDailyWhen(new Date(), when);
    const SUFFIX = "Daily-log";
    const FILE_EXT = "md";
